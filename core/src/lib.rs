...
impl WhisperContext {
    pub fn new(model_path: &str) -> Result<Self, CoreError> {
        ...
        let cparams = unsafe { bindings::whisper_context_default_params() };

        let ctx = unsafe { bindings::whisper_init_from_file_with_params(c_path.as_ptr(), cparams) };
        if ctx.is_null() {
    ...
    fn build_params(&self, opts: &TranscribeOptions) -> bindings::whisper_full_params {
        let mut params = unsafe {
            bindings::whisper_full_default_params(
                bindings::whisper_sampling_strategy_WHISPER_SAMPLING_GREEDY,
            )
        };

        params.n_threads = if opts.n_threads > 0 {
            opts.n_threads
        } else {
    ...
    pub fn transcribe_segments(
        ...
        for i in 0..n {
            let ptr = unsafe { bindings::whisper_full_get_segment_text(self.ctx, i) };
            if ptr.is_null() {
            ...
            let start_ms = unsafe { bindings::whisper_full_get_segment_t0(self.ctx, i) };
